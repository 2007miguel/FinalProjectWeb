...
public class UsersJpaController implements Serializable {
        ...
        try {
            ...
            if (msg == null || msg.length() == 0) {
                ...
                if (findUsers(id) == null) {
                    throw new NonexistentEntityException("The user with username " + id + " no longer exists.");
                }
            }
            throw ex;
        } finally {
    ...
    public void destroy(String username) throws NonexistentEntityException {
        ...
        try {
            ...
            Users user;
            try {
                user = em.getReference(Users.class, username);
                user.getUsername();
            } catch (EntityNotFoundException enfe) {
                throw new NonexistentEntityException("The user with username " + username + " no longer exists.", enfe);
            }
            em.remove(user);
            em.getTransaction().commit();
        } finally {
